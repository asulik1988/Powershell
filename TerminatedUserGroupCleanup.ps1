...
$disabledUsers = (Get-ADUser -SearchBase "OU=Disabled Users,DC=contoso,DC=inc" -Filter {enabled -eq $false} -Properties Company | `
where {$_.Company -in $rtsCompanies})

#Log Formatting
$logLocation = Read-Host "Please enter where to save log.. example: C:\DisabledUsers.txt"
$Title = "Contoso Disabled User Cleanup"
$space = "                                                                           "
$tab = "     "
...
if ($_.distinguishedName.contains("OU=Groton,DC=contoso,DC=inc" -or "OU=Metairie,DC=contoso,DC=inc")){
